...
AGG_TICKIT_SALES_BY_CATEGORY = f"""
    CREATE TABLE IF NOT EXISTS agg_tickit_sales_by_catorgery
    WITH (
        format = 'Parquet',
        write_compression = 'SNAPPY',
        external_location = 's3://{S3_BUCKET}/tickit/gold/tickit_sales_by_catergory/',
    ...
    AS WITH as AS (
        SELECT DISTINCT e.eventid,
            c.catgroup,
            c.catname
        FROM refined_tickit_public AS e
